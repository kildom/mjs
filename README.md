...
- Source code parsing functions are not needed on the target device, so MJS engine code size is reduced.
- Bytecode can be located in a flash memory, so MJS engine consumes less RAM.

## Downbacks
This approach causes some significant implications:
- Arbitrary bytecode execution is not secure, so it has to come from a trusted source.
- The compilation has to be done before it goes to the target device with some tool.
  This tool needs a compatible version of the MJS.
- Bytecode is bigger than the minified original source code.
- Original source code cannot be generated from a bytecode, so it has to be saved in some different way.
- `load` buildin is not working.

## API additions
- `mjs_compile` function compiles the source code and returns buffer with a bytecode.
- `mjs_exec_compiled` function adds provided bytecode to a mjs context and executes it.
...
- Javascript library allowing compilation of a source code in a browser. This allows bytecode handling with a web interface.
  It will be created with Emscripten.
- Merging repeating strings in a bytecode into one to reduce bytecode size.
- Move debug/diagnostic information (i.e. information needed for backtrace) from bytecode to a different location to reduce bytecode size.
