...
- Bytecode can be located in flash memory, so MJS engine consumes less RAM.
...
- Original source code cannot be generated from bytecode, so it has to be saved in some different way.
- `load` buildin is not working.

## API changes
- `mjs_exec*` functions are no longer available.
- `mjs_compile` function compiles the source code and returns buffer with a bytecode.
- `mjs_exec_compiled` function adds provided bytecode to a context and executes it.

## Future plans
- `mjs_compress_src` and `mjs_decompress_src` functions will handle a source code lossless compression.
  It is pretty efficient, because it takes strings from generated bytecode and MJS keywords as an initial dictionary.
  Compressed source code can be saved together with a bytecode allowing source code to be recovered easily.
